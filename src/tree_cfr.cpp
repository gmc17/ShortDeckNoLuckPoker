...
Tree cfr_plus(int iterations, const std::array<std::array<float, NUM_CARDS>, NUM_CARDS> op_range, 
    ...
    for (int t=1; t<=iterations; t++) {
        ...
        cfr_plus_traverse_tree(tree.get_root(), 0, weight, ones, ip_range, op_range, pool);
        cfr_plus_traverse_tree(tree.get_root(), 1, weight, ones, op_range, ip_range, pool);

        if (t%50==0) {
            std::cout << "Iteration " << t << " complete.\n";
            calculate_exploitability(tree, op_range, ip_range, board_cards, pot_size);
        }
    }
...
std::unique_ptr<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>> cfr_plus_traverse_tree( 
    ...
    if (!node) return std::make_unique<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>>();

    if (node->is_terminal_node()) return terminal_node_utility(node, traversing_player, reach_probabilities, inactive_player_range, traversing_player_range);
    ...
    if (node->is_chance_node()) {
        ...
        for (int c=1; c<=NUM_CARDS; c++) {
            if (chance_node.has_card(c)) continue;
            futures.push_back(pool.enqueue([&, c]() {
                return cfr_plus_traverse_tree(chance_node.children[c - 1].get(), traversing_player, weight, reach_probabilities, inactive_player_range, traversing_player_range, pool);
            }));
        }
        ...
        for (int c1=1; c1<=NUM_CARDS; c1++) {
            for (int c2=c1+1; c2<=NUM_CARDS; c2++) {
                (*info_set_utilities)[c1 - 1][c2 - 1] /= (num * 1.0f);
            }
        }
    ...
    if (decision_node.player == traversing_player) {
        ...
        for (int a=0; a<actions; a++) {
            auto temp = cfr_plus_traverse_tree(decision_node.children[a].get(), traversing_player, weight, reach_probabilities, inactive_player_range, traversing_player_range, pool);
            (*action_vals)[a] = *temp;
        }
        for (int c1=1; c1<=NUM_CARDS; c1++) {
            for (int c2=c1+1; c2<=NUM_CARDS; c2++) {
                if (!(decision_node.has_card(c1) || decision_node.has_card(c2))) {
                    ...
                    const auto& strategy = decision_node.get_strategy(c1, c2);
                    for (int a=0; a<actions; a++) {
        ...
        for (int a=0; a<actions; a++) {
            ...
            auto temp = cfr_plus_traverse_tree(decision_node.children[a].get(), traversing_player, weight, *new_reach_probabilities, inactive_player_range, traversing_player_range, pool);
...
std::unique_ptr<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>> terminal_node_utility(const Tree::Node* node, 
                                                                                                         bool traversing_player, 
...
std::unique_ptr<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>> update_reach_probabilities(const Tree::Node* node, 
                                                                                         int action, 
                                                                                         const std::array<std::array<float, NUM_CARDS>, NUM_CARDS>& reach_probabilities) {
...
std::unique_ptr<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>> node_update_chance_reach_probabilities(const Tree::Node* node, 
                                                                                                            int num, 
                                                                                                            const std::array<std::array<float, NUM_CARDS>, NUM_CARDS>& reach_probabilities) {
    auto res = std::make_unique<std::array<std::array<float, NUM_CARDS>, NUM_CARDS>>();
